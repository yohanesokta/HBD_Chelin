document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- SCROLL-BASED ANIMATIONS & FEATURES ---
    setupAnimations();
    setupInteractiveElements();

    function setupAnimations() {
        const animateOnScroll = (elem, vars) => gsap.utils.toArray(elem).forEach(el => gsap.from(el, { scrollTrigger: { trigger: el, start: 'top 85%', toggleActions: 'play none none none' }, ...vars }));
        ...
        gsap.utils.toArray('.parallax-bg').forEach(bg => {
            gsap.to(bg, { y: (i, target) => -(target.parentElement.offsetHeight - target.offsetHeight) * 0.4, ease: "none", scrollTrigger: { trigger: bg.parentElement, start: "top top", end: "bottom top", scrub: true } });
        });
        ...
        gsap.utils.toArray("section").forEach(section => {
            ...
            if (color) {
                ScrollTrigger.create({
                    ...
                    onEnter: () => gsap.to('body', { backgroundColor: color, duration: 1.2, ease: 'sine.inOut' }),
                    onEnterBack: () => gsap.to('body', { backgroundColor: color, duration: 1.2, ease: 'sine.inOut' })
                });
            }
        });
    ...
    function setupInteractiveElements() {
        ...
        if (loveChartCanvas && typeof Chart !== 'undefined') {
            ScrollTrigger.create({
                trigger: loveChartCanvas, start: 'top 80%', once: true,
                onEnter: () => {
        ...
        if(cube && typeof Draggable !== 'undefined') {
            let rotX = -30, rotY = -45;
            ...
            Draggable.create(cube, {
                onDragStart: function() { autoRotate.pause(); gsap.to(this.target.querySelectorAll('.cube-face'), { boxShadow: "0 0 40px var(--glow-cyan) inset", duration: 0.3 }); },
                onDrag: function() { rotY += this.deltaX * 0.5; rotX -= this.deltaY * 0.5; gsap.to(this.target, { rotationX: rotX, rotationY: rotY, duration: 0.1, ease: 'power1.out' }); },
                onDragEnd: function() { autoRotate.resume(); gsap.to(this.target.querySelectorAll('.cube-face'), { boxShadow: "0 0 20px var(--glow-cyan) inset", duration: 0.5 }); }
            });
        }

        const quizContainer = document.getElementById('quizContainer');
        if (quizContainer) {
            const quizContent = document.getElementById('quizContent'), quizProgress = document.getElementById('quizProgress'), quizQuestion = document.getElementById('quizQuestion'), quizAnswers = document.getElementById('quizAnswers'), quizResultScreen = document.getElementById('quizResultScreen'), quizScoreEl = document.getElementById('quizScore'), quizResultMessage = document.getElementById('quizResultMessage');
            const quizData = [
            ...
            function loadQuestion() {
                ...
                currentQuestion.answers.forEach((answer, index) => { const button = document.createElement('button'); button.textContent = answer; button.classList.add('answer-btn'); button.dataset.index = index; button.addEventListener('click', selectAnswer); quizAnswers.appendChild(button); });
            }
            function selectAnswer(e) {
                const selectedButton = e.target, selectedIndex = parseInt(selectedButton.dataset.index), correctIndex = quizData[currentQuestionIndex].correct;
